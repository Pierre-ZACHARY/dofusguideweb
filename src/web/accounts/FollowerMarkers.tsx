...
export function FollowerAvatar({
  profile,
  className = "h-7 w-7",
}: Readonly<{ profile: FollowedProfile; className?: string }>) {
  return (
...
export function FollowerAvatarStack({ profiles }: Readonly<{ profiles: FollowedProfile[] }>) {
  if (profiles.length === 0) return null;
  return (
    <span className="relative z-[60] flex -space-x-2 overflow-visible" aria-label={profiles.map((profile) => profile.name).join(", ")}>
      {profiles.slice(0, 4).map((profile) => <FollowerAvatar key={profile.id} profile={profile} />)}
...
export function FollowerProgressMarkers({
  profiles,
  percentFor,
}: Readonly<{ profiles: FollowedProfile[]; percentFor: (profile: FollowedProfile) => number }>) {
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 h-11 overflow-visible" aria-label={profiles.length > 0 ? "Position des profils suivis" : undefined}>
      {profiles.map((profile, index) => (
        <span
          ...
          key={profile.id}
        >
          <FollowerAvatar profile={profile} className="h-8 w-8" />
